...
export default async function SanityAvailabilityHero() {
  ...
  return (
    <div 
      ...
      style={{
        backgroundImage: "url('/mitsurin-wagyu-pasture-rasied-hondo-texas.webp')",
      }}
    >
      {/* Lighter overlay */}
      ...
      <section
        ...
        <h1 className="text-4xl sm:text-5xl font-bold font-[Montserrat] leading-tight" data-sanity-edit-target>
          <span className="text-accent-dark drop-shadow-lg">{pageTitle.split(' ')[0]} </span>
          <span className="text-white drop-shadow-lg">{pageTitle.split(' ').slice(1).join(' ')}</span>
        </h1>

        {/* Description */}
        <p className="font-[Montserrat] mt-6 text-white text-2xl font-medium max-w-3xl drop-shadow-md" data-sanity-edit-target>
          {pageDescription}
        </p>
      </section>
    </div>
