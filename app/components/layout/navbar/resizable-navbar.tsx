...
export const Navbar = ({
  ...
  useMotionValueEvent(scrollY, "change", (latest) => {
    ...
    else if (latest > 100) setIsVisible(true);
  ...
  return (
    <motion.div
      ref={ref}
      // IMPORTANT: Change this to `fixed` if you want the navbar to be fixed
...
export const NavBody = ({ children, className, isVisible }: NavBodyProps) => {
  return (
    <motion.div
      animate={{
      ...
      className={cn(
        ...
        isVisible && "bg-white/30 dark:bg-white/30",
        className
      )}
    >
      {React.Children.map(children, (child) =>
    ...
    <motion.div
      ...
      className={cn(
        ...
        isVisible
          ...
          : "text-neutral-200 hover:text-stone-900",
        className
      )}
    >
      {items.map((item, idx) => (
        <a
          ...
          className={cn(
            ...
            isVisible ? "text-stone-900 dark:text-stone-900" : "text-neutral-200"
          )}
          key={`link-${idx}`}
          href={item.link}
        >
          {hovered === idx && (
            <motion.div
              ...
              className={cn(
                ...
                isVisible ? "bg-gray-200" : "bg-neutral-800"
              )}
            />
          )}
...
export const MobileNav = ({ children, className, isVisible }: MobileNavProps) => {
  return (
    <motion.div
      animate={{
        backdropFilter: isVisible ? "blur(10px)" : "none",
        ...
        borderRadius: isVisible ? "4px" : "2rem",
        y: isVisible ? 20 : 0,
      }}
      transition={{
        type: "spring",
  ...
  return isOpen ? (
    <IconX
      className={cn(
        ...
        isVisible
          ? "text-stone-800 dark:text-stone-800"
  ...
  return (
    ...
    >
      <img
        src={
          isVisible
            ? "/mitsurin-horizontal-logo-black.png"
            : "/mitsurin-horizontal-logo-white.svg"
        }
